...
class MVSwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=384, patch_size=4, in_chans=3, num_classes=1,
        ...
        # ------------ FIX dynamic fused linear dimension -------------
                
        # num layers effettive nella parte multiview (quelle che abbiamo appena aggiunto)
        num_multiview_layers = len(self.layers)  # es: self.num_layers // 2

        # calcola il numero di patch per lato dopo tutte le patch-merges in self.layers
        # ogni PatchMerging dimezza H e W, quindi moltiplichiamo per 2**num_multiview_layers
        tokens_per_side_after = patches_resolution[0] // (2 ** num_multiview_layers)
        tokens_per_side_after_w = patches_resolution[1] // (2 ** num_multiview_layers)

        if tokens_per_side_after < 1 or tokens_per_side_after_w < 1:
            raise ValueError(f"Too small spatial resolution after multiview layers. "
                            f"patches_resolution={patches_resolution}, num_multiview_layers={num_multiview_layers}")

        # token count per vista (N)
        tokens_per_view = tokens_per_side_after * tokens_per_side_after_w

        # input alla fc sarà 2 * N (concat CC + MLO lungo la dimensione dei token)
        fused_in_dim = tokens_per_view * 2
        # usiamo fused_out_dim = tokens_per_view (così ritorniamo ad N token per l'input alle layers_fused)
        fused_out_dim = tokens_per_view
    ...
    def forward_features(self, x1, x2):
        ...
        for layer in self.layers:
            ...
            for blk in layer.blocks:
                self.attentions_cc.append(blk.attn.attn_map_1)
                self.attentions_mlo.append(blk.attn.attn_map_2)
...
class SwinTransformer_singleview(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers:
            ...
            for blk in layer.blocks:
                self.attentions.append(blk.attn.attn_map)  
        x = self.norm(x)
        x = self.avgpool(x.transpose(1, 2))
        x = torch.flatten(x, 1)
        return x
