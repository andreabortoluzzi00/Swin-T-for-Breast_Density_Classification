...
def split_train_val(csv_path, out_dir, group_col="patient_id", test_size=0.3, seed=5042):
    ...
    print("Class distribution (val):\n", val_df['breast_density'].value_counts())
    return train_csv, val_csv


# --------------------- base dataset ---------------------------------------

class _BaseCBISDataset(Dataset):
    """Base CBIS-DDSM dataset with common utilities."""
    ...
    def __init__(self, df: pd.DataFrame, images_root: str, img_size: int = 224, augment: bool = False):
        ...
        if self.augment:
            self.aug_transform = Compose([
                ...
                RandRotate(range_x=10, prob=0.7, keep_size=True, mode='bilinear'),
                RandZoom(min_zoom=0.9, max_zoom=1.1, prob=0.7, keep_size=True, mode='bilinear'),
            ])
        else:
...
class CBISSingleViewDataset(_BaseCBISDataset):
    """Single-view dataset: each row is one image."""
    ...
    def __init__(self, csv_path=None, df: pd.DataFrame = None, images_root=None, img_size=224, augment=False):
        ...
        required = ['expected_file', 'breast_density', 'patient_id', 'image view', 'left or right breast']
        for c in required:
    ...
    def __getitem__(self, idx):
        ...
        img = self._load_image_tensor(row['expected_file'])
        label = int(row['breast_density'])
        meta = {
...
class CBISMultiViewDataset(_BaseCBISDataset):
    ...
    def _generate_pairs(self):
        ...
        for patient_id, group in self.grouped:
            for lat in ['LEFT', 'RIGHT']:
                lat_group = group[group['left or right breast'] == lat]
                cc = lat_group[lat_group['image view'] == 'CC']
                mlo = lat_group[lat_group['image view'] == 'MLO']

                if len(cc) > 0 and len(mlo) > 0:
                    for cc_row, mlo_row in product(cc.to_dict('records'), mlo.to_dict('records')):
                        pairs.append((cc_row['expected_file'], mlo_row['expected_file']))
                        labels.append(int(cc_row['breast_density']))
                        metas.append({'patient_id': patient_id, 'laterality': lat})
                else:
                    if self.keep_incomplete:
                        if len(cc) > 0:
                            for cc_row in cc.to_dict('records'):
                                pairs.append((cc_row['expected_file'], None))
                                labels.append(int(cc_row['breast_density']))
                                metas.append({'patient_id': patient_id, 'laterality': lat})
                        if len(mlo) > 0:
                            for mlo_row in mlo.to_dict('records'):
                                pairs.append((None, mlo_row['expected_file']))
                                labels.append(int(mlo_row['breast_density']))
                                metas.append({'patient_id': patient_id, 'laterality': lat})
...
def get_cbis_dataloader(csv_path, images_root, out_dir, view_mode='multi', batch_size=32, img_size=224,
    ...
    if view_mode == 'single' and view_type is not None:
        train_df = train_df[train_df['image view'] == view_type].reset_index(drop=True)
        val_df = val_df[val_df['image view'] == view_type].reset_index(drop=True)

    if view_mode == 'single':
        train_ds = CBISSingleViewDataset(df=train_df, images_root=images_root, img_size=img_size, augment=augment)
        val_ds = CBISSingleViewDataset(df=val_df, images_root=images_root, img_size=img_size, augment=False)
        collate = collate_single
    elif view_mode == 'multi':
        train_ds = CBISMultiViewDataset(csv_path=train_csv, images_root=images_root, img_size=img_size, augment=augment, keep_incomplete=keep_incomplete)
        val_ds = CBISMultiViewDataset(csv_path=val_csv, images_root=images_root, img_size=img_size, augment=False, keep_incomplete=keep_incomplete)
        collate = collate_multi
    else:
        raise ValueError("view_mode must be 'single' or 'multi'")

    if use_weighted_sampler:
        labels = train_ds.labels if hasattr(train_ds, 'labels') else train_ds.df['breast_density'].tolist()
...
if __name__ == '__main__':
    csv_path = 'thesis/datasets/cbis/cbis_ddsm_full_clean.csv'
    images_root = 'thesis/datasets/cbis/cropped_png'
    out_dir = './dataset_cbis'

    train_loader, val_loader, train_ds, val_ds = get_cbis_dataloader(
        csv_path=csv_path,
    ...
    print('Train class counts :', Counter([int(x) for x in (train_ds.labels if hasattr(train_ds, 'labels') else train_ds.df['breast_density'].tolist())]))
    print('Val class counts :', Counter([int(x) for x in (val_ds.labels if hasattr(val_ds, 'labels') else val_ds.df['breast_density'].tolist())]))
