...
# ---------------------------------------------------------------------------
# Script purpose:
# - Automatically crop the breast region from pre-cleaned mammography images
# - Remove large background areas by detecting the largest continuous
#   non-zero variance region along rows and columns
# - Save cropped images for downstream training
# ---------------------------------------------------------------------------
...
def np_CountUpContinuingOnes(b_arr):
    ...
    """
    Compute the length of the longest continuous segment of True values
    for each position in a boolean array.

    Used to detect the widest non-empty region along an axis.
    """
...
def adjust_bounding_box(original_coords, left_crop, top_crop):
    ...
    left_crop, top_crop: amount removed from left and top
    """
...
def ExtractBreast(img, true_bounding_box):
    ...
    """
    Extract the main breast region from a mammography image.

    The method:
    - Suppresses low-intensity background
    - Finds the widest continuous region with non-zero variance
